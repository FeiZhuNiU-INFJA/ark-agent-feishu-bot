...
**所有产物都落在项目根的 `ma-cases/<case>/` 下**，一个 case（= 一次「拿某客户某批轨迹做复刻实验」）
一个独立目录，互不干扰。这套布局由冻结层 [scripts/case_paths.py](scripts/case_paths.py) 钉死，
所有脚本共用 `--case-dir` 一个开关：
...
- 一条命令（最终口径）：
  ```bash
  python run.py --case-dir ../../../ma-cases/<case> --mock both --all --repeats 5
  ```
