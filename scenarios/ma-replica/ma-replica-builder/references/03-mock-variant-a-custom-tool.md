...
- 未命中 → 返回结构化 `[tool_error] REPLAY_MISS`，让模型**如实感知"走出了录制轨迹"**，
  绝不喂假数据（喂假数据会污染对比）。
- 回放键归一化在客户样板 `example-nio/scripts/replay_lib.py` 的 `api_call_key()`：把"决定返回哪行
  数据"的参数纳入键，忽略分页/排序/列顺序等不影响命中的字段。**换客户改这里**（客户特有）。

## 冻结引擎 + 客户工具面（分层）

事件循环、指标累计、资源清理都在**冻结层** `scripts/ma_runtime.py`，不随客户变。变体 A 客户侧
只需在 `example-nio/scripts/run.py` 提供两小块：
...
python example-nio/scripts/run.py --case-dir ../ma-cases/nio --mock custom --model doubao-seed-2-1-pro-260628
