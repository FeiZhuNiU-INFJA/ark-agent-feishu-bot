...
─────────────── 中间产物契约（下面钉死的那几个 JSON） ───────────────
                          ▲ 由客户层生成
③ 客户·生成式代码   example-nio/scripts/extract_trajectories.py ·      依赖客户轨迹
...
> `replay_map.json` 的 key 由客户层用**客户接口语义**算出来（NIO 在 `replay_lib.api_call_key()`），
...
> 这两个曾经放在 `example-nio/scripts/`，是历史误置。它们从不读原始轨迹，只读契约，
> 所以上升进冻结层——**换客户直接调用，不用改**。

## 层③ 客户·生成式代码 `example-nio/scripts/`（依赖客户轨迹，现写）

**不是"复制模板改几行"，而是"读懂这个客户的轨迹后，照 example-nio 现写"。**
`example-nio/` 是一份**参照实现**，示范"层③应该长什么样、产出什么契约"，不是可配置的通用件。
...
3. 照 `example-nio/scripts/` **现写**层③：`extract` 认这个客户的结构并吐出契约、`replay_lib.api_call_key()`
