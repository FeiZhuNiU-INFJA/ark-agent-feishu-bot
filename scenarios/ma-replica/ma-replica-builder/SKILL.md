...
> 都不同，**把原始轨迹拆成中间产物**的那段代码要由你分析完这个客户的轨迹后**现写**（照 `example-nio/` 写）。
...
> ⚠️ 轨迹导出**是否含 token/cache 取决于客户格式**。很多导出（含本例 NIO 6 条，已逐字段核实）
> **只有 `messages`、没有 `usage`**，此时自研侧 token/cache **无法恢复**，对比报告里这两项只给
> MA 单边实测。要双边 token 对比，须向客户额外要「含 `usage` 的全量请求导出」。

## 工作目录规范（一个 case 一个目录）

**所有产物都落在本 skill 的兄弟目录 `../ma-cases/<case>/`（即 `scenarios/ma-replica/ma-cases/<case>/`）下**，
一个 case（= 一次「拿某客户某批轨迹做复刻实验」）一个独立目录，互不干扰。
下文命令均**以 skill 根 `scenarios/ma-replica/ma-replica-builder/` 为工作目录**，故 case 目录写作 `../ma-cases/<case>`。
...
- **`../ma-cases/` 不入库**：它是 per-case 运行期数据（已在 `.gitignore`）；skill 自带的参照样板在
  `example-nio/data/`，那份才进版本库、当回归基准。
...
- 一条命令（最终口径，cwd = skill 根 `scenarios/ma-replica/ma-replica-builder/`）：
  ```bash
  python example-nio/scripts/run.py --case-dir ../ma-cases/<case> --mock both --all --repeats 5
  ```
...
- **③ 客户·生成式代码 `example-nio/scripts/`（依赖客户轨迹，现写）**：
...
照 `example-nio/scripts/` **现写**层③（把轨迹拆成契约），**不是复制模板改几行** →
...
`example-nio/scripts/run.py` 用 `--mock files`（默认）/ `--mock custom` 一个开关切换两种模式。

## Worked example（NIO 6 条轨迹）

`example-nio/` 是拿现成 6 条蔚来销售盘点轨迹跑通的完整例子（含层③ 客户生成式脚本的参照实现），见
[example-nio/README.md](example-nio/README.md)。踩坑与口径边界见 [PROBLEMS.md](PROBLEMS.md)。
