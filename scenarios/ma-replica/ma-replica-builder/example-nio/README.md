...
`context/trajectory/trajectory{1..6}.json`（在 `scenarios/ma-replica/context/trajectory/`；每个是一次自研 Agent 的完整运行，顶层 `messages[]`）。
...
# 准备 case 工作目录：把 6 条原始轨迹放进兄弟目录 ../ma-cases/nio/trajectories/
mkdir -p ../ma-cases/nio/trajectories
cp ../context/trajectory/trajectory*.json ../ma-cases/nio/trajectories/
...
export ARK_API_KEY=...
cd scenarios/ma-replica/ma-replica-builder
python example-nio/scripts/run.py --case-dir ../ma-cases/nio --model doubao-seed-2-1-pro-260628 --all   # 默认 files 模式
# 或高保真：python example-nio/scripts/run.py --case-dir ../ma-cases/nio --mock custom --all
