...
用法（cwd = skill 根 scenarios/ma-replica/ma-replica-builder/）：
    export ARK_API_KEY=...
    python example-nio/scripts/run.py --case-dir ../ma-cases/nio                 # 默认 files 模式，跑第一条
    python example-nio/scripts/run.py --case-dir ../ma-cases/nio --mock custom --all
    python example-nio/scripts/run.py --case-dir ../ma-cases/nio --mock both --all --repeats 5   # 最终交付口径
    python example-nio/scripts/run.py --out-dir ../data                          # 旧口径（平铺目录）
"""
...
def resolve_layout(a: argparse.Namespace, mock_mode: str) -> tuple[Path, Path, Path, Path]:
    ...
    out = Path(a.out_dir)
    traj_dir = Path(a.traj_dir) if a.traj_dir else (HERE.parents[2] / "context/trajectory")
