"""replay_lib.py —— 客户层（本例：Acme 客服平台）：业务接口回放键 + 严格回放的纯函数。

被同目录的 extract_trajectories.py / run.py 复用。单处定义，避免"抽取端"与"回放端"键规则漂移。

★★★ 全文都是客户特有逻辑：api_call_key() 的字段语义是 Acme 工单系统 `api_call` 网关的接口形态。
...
def api_call_key(args: dict) -> str:
    ...
    ── 客户特有 ──
    这里的参数字段（object_code / recommend_tag / where.conditions ...）是 Acme 工单系统的形态。
